...
for NODE in ${BFT_NODES}; do
  (
    ...
    echo "  --topology                        ../${NODE}/topology.json \\"
    echo "  --database-path                   ../${NODE}/db \\"
    echo "  --socket-path                     ../${NODE}/node.sock \\"
    echo "  --shelley-kes-key                 ../${NODE}/shelley/kes.skey \\"
    echo "  --shelley-vrf-key                 ../${NODE}/shelley/vrf.skey \\"
    echo "  --shelley-operational-certificate ../${NODE}/shelley/node.cert \\"
    echo "  --port                            $(cat ${NODE}/port) \\"
    echo "  --delegation-certificate          ../${NODE}/byron/delegate.cert \\"
    echo "  --signing-key                     ../${NODE}/byron/delegate.key \\"
    echo "  | tee -a ../${NODE}/node.log"
  ) > run/${NODE}.sh
